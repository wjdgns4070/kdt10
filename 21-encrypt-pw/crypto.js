...
function saltAndHashPw(password){
    ...
    const hash = crypto.pbkdf2Sync(password,salt,iteration,keylen,algorithm).toString('base64'); // buffer -> string 으로 반환
...
function checkPW(inputPW, savedSalt, savedHash) {
    ...
    const hash = crypto.pbkdf2Sync(inputPW,savedSalt,iteration,keylen,algorithm).toString('base64'); // buffer -> string 으로 반환

    return savedHash === hash; // 같다면 true, 다르면 false 반환
}
